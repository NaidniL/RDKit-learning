...
| 所有来源记录均为 `model_structure_ok=false` | `ineligible` | 每个阶段 1 结构原因加前缀 `stage1_structure_excluded:` |
...
人工输入本身必须符合第 10 节编码、列顺序和 `compound_id,dataset_role` 排序规范。正式 release
必须包含 `modeling/conflict_reviews.csv`，schema 与人工输入相同，并直接复制已经验证的输入字节；
它与最终人工输入 SHA-256 完全一致，恰好覆盖 614 个冲突，并进入 manifest 和确定性 artifacts。
`reports/label_conflicts.csv` 必须通过 compound ID 和 dataset role 精确 join，逐项带出
`decision/review_reason/reviewer/reviewed_at_utc`；只有文件哈希而无逐项证据不满足冻结要求。

首次及后续 audit 均生成 schema-registry 登记的 audit-only 文件
`reports/modeling_conflict_review_candidates.csv`，字段固定为：

```text
compound_id,dataset_role,clear_positive_count,clear_negative_count,
record_keys_json,review_status,decision,review_reason,reviewer,reviewed_at_utc
```

按 `compound_id,dataset_role` 排序。它进入 audit manifest 的 `audit_only_artifacts`，不进入正式
release，也不参与正式 release artifact 集合比较；最终 audit 中仍保留它作为审核归档。
